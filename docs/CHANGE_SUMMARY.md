...
本文档用于后期定位“为什么改、改了哪里、正常路径是否变化、如何验证”。比较对象是合并前的
ROS 2 基础实现 `6417c63` 及原 LIO/LVI-SAM 数据链；当前正式交付基线为 `main` 的合并提交
`f140df2`。后续提交若只修改文档，应继续记录实际部署提交，但不改变这里列出的算法边界。

版本锚点（用于复现和排查）：

- **正式增强基线**：`19ca76a`，包含完整增强功能，并最终确认定位状态与复位事件均为观测输出；
- **`main` 合并基线**：`f140df2`，将 `19ca76a` 合入默认分支，同时保留旧 `main` 的 ROS
  `setup.bash`/`set -u` 兼容修复；
- **状态接口分阶段基线**：`d87189f`，只包含观测型 `LocalizationStatus`，可用于历史 A/B；
- **历史联动基线**：`4bb5ca5` 曾把 `LocalizationReset` 接入 IMU/VIS 复位；实机发现该做法
  改变了原算法时序，现已撤销内部订阅，事件恢复为观察性输出；
- `88f2b7e` 恢复注册稠密点云发布，`d8b03b2`/`d7f215f` 修正 ROS 2 Humble 时间与 QoS
  兼容，`22ba687` 取消定位模式处理间隔，`19ca76a` 将状态输出与估计器内部控制彻底解耦。
  这些均属于正式增强基线，不应再回退到 `4bb5ca5` 作为生产版本。

审阅更新日期：2026-08-19。远端默认维护分支已经是 `main`；
`agent/visual-rviz-defaults` 仅作为合并前历史备份。新部署直接克隆默认分支，已有部署切换到
`main` 后，应使用 `git rev-parse HEAD` 和 `git status --short` 记录实际版本及本地改动。

### 交付结论

本版本不是对原 LVI-SAM 优化器的整体重写，而是在保留 LOAM 特征、scan-to-map、IMU 预积分、
GTSAM 因子图、VINS 滑窗和 BRIEF/DBoW2 主体计算的前提下，补齐 ROS 2 工程化、MID-360 接入、
建图/定位模式、先验地图持久化、传感器配置、输入防护、状态接口和部署诊断。需要特别区分：

- **估计结果发生变化的改动**：正确外参、安装倾角初始化、完整 Livox 点时间处理、每帧定位处理、
  先验地图约束和可选 RTK 门控；这些改变是为修正输入语义或启用明确的新模式。
- **不应改变正常估计结果的改动**：状态/复位消息、配置校验、队列边界、非法样本拒绝、RViz、
  文档和脚本；这些只提高可观测性、故障隔离和部署一致性。
- **当前尚未实现的能力**：跨会话视觉地图加载。相机数据的持久化格式已经设计，但当前正式
  跨会话重定位仍以 Scan Context 候选与点云 ICP 几何验证为主。
