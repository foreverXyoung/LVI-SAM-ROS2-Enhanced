...
- `/lio_sam/localization/status` 与 `/lio_sam/localization/reset` 都是面向上层的观察性输出。
- 发布状态或事件不会清空 IMU/VIS 队列，不会重建 GTSAM 图，不会把 IMU bias 归零，也不会重启视觉滑窗。
- 内部兼容复位仍沿用原 LIO-SAM 的 mapping odometry `reset_id` 路径；事件发布函数不递增该代次。
- 保留零时长 IMU 预积分防护：没有有效 IMU 区间时跳过本次图更新并等待下一帧，避免 `b0` 奇异导致进程退出。
...
- 重定位复位后，IMU 预积分在没有有效 post-reset 时间区间时跳过零时长图因子；GTSAM 欠约束异常会
  清理短期传播状态并发布 IMU 失败复位事件，不再直接终止 `lvi_sam_imuPreintegration`。
