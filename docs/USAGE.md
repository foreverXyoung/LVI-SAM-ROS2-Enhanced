...
> LIS 使用 livox `CustomMsg`（`/livox/lidar`、`/IMU_data`）；
> VIS 使用标准 `sensor_msgs/Imu`（默认 `imu_topic: /livox/imu`，
> livox_ros_driver2 会同时发布 `/livox/imu` 标准消息）。**二者无需额外转换节点。**
...
```bash
ros2 launch lvi_sam run.launch.py \
  lidar_params_file:=$(ros2 pkg prefix lvi_sam)/share/lvi_sam/config/params_gazebo_localization.yaml \
...
| 参数 | 默认 | 说明 |
|------|------|------|
| `lidar_params_file` | `config/params.yaml` | LIS 参数；可换 `params_gazebo_localization.yaml` / `params_charging_localization.yaml` 等 |
...
| `publish_map_odom_static` | `true` | 是否发布 `map→odom` 静态变换（独立跑 SLAM 时建议 `true`，接入导航栈时可关） |
...
| `/livox/imu` | `sensor_msgs/Imu` | VIS 输入（标准 IMU） |
| `/IMU_data` | `livox_ros_driver2/msg/CustomMsg` | LIS 输入（livox IMU） |
| `/lio_sam/odometry/imu` | `nav_msgs/Odometry` | **LIS→VIS** 位姿+尺度先验（实际解析为 `/odometry/imu`） |
...
- **回环**：`loopClosureEnabled`（ScanContext 开关）、`externalLoopClosureSource`
  （外部回环，即 VIS 接入的 `lio_loop/loop_closure_detection`）。
- **地图**：`savePCD`、`savePCDDirectory`、`Loc.loadPCDDirectory`（先验地图）。
- **RTK / 重定位**：`gpsTopic`、`useImuHeadingInitialization`、`LocInitSta` 等。

场景变体：`params_gazebo_*.yaml`（仿真）、`params_charging_*.yaml`（充电场景）、
`*_localization.yaml` / `*_mapping.yaml`（定位/建图模式）。切换只需改 `lidar_params_file` 参数。
...
- `imu_topic: /livox/imu`、`image_topic: /camera/image_raw`。
...
1. 用仿真或录制的 bag 提供 `/livox/lidar`、`/livox/imu`、`/camera/image_raw`。
...
| VIS 完全不动 / 无位姿 | `ros2 topic hz /livox/imu`、`/camera/image_raw`、`/odometry/imu` 是否都有数据 |
...
| 激光驱动话题名不符 | 不同 livox launch 话题名可能不同，按实机调整 `lidarTopic` / `imuTopic` |

---

## 8. 调参与后续

- **对称/长直场景误闭环**：站场高度对称，ScanContext 与视觉回环都可能误匹配。
  建议在 `mapOptmization` 的外部回环分支加 **RTK / 先验地图一致性门控**再接受回环（本工程预留了接口）。
- **VIS→LIS 前端初值（③a）**：`imu_propagate_ros` 暂未接入（最小闭环不需要），可作为精度增强项后续补。
- **多场景切换**：用 `lidar_params_file` 切换 `params_gazebo_*` / `params_charging_*`，
  无需改代码。
