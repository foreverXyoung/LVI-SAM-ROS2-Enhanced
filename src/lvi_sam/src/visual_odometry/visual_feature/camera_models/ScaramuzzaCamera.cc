...
bool
OCAMCamera::Parameters::readFromYamlFile(std::shared_ptr<rclcpp::Node> node, const std::string& filename)
{
    ...
    for(int i=0; i < SCARAMUZZA_POLY_SIZE; i++){
        ...
        std::string temp = std::string("poly_parameters.p") + boost::lexical_cast<std::string>(i);
        node->declare_parameter(temp, 1.0);
        node->get_parameter(temp, m_poly[i]);
    }
    ...
    for(int i=0; i < SCARAMUZZA_INV_POLY_SIZE; i++){
        ...
        std::string temp = std::string("inv_poly_parameters.p") + boost::lexical_cast<std::string>(i);
        node->declare_parameter(temp, 1.0);
        node->get_parameter(temp, m_inv_poly[i]);
    }

    // n = fs["affine_parameters"];
    node->declare_parameter("affine_parameters.ac", 1.0);
    node->get_parameter("affine_parameters.ac", m_C);
    node->declare_parameter("affine_parameters.ad", 1.0);
    node->get_parameter("affine_parameters.ad", m_D);
    node->declare_parameter("affine_parameters.ae", 1.0);
    node->get_parameter("affine_parameters.ae", m_E);
    // m_C = static_cast<double>(n["ac"]);
    // m_D = static_cast<double>(n["ad"]);
    // m_E = static_cast<double>(n["ae"]);

    node->declare_parameter("affine_parameters.cx", 1.0);
    node->get_parameter("affine_parameters.cx", m_center_x);
    node->declare_parameter("affine_parameters.cy", 1.0);
    node->get_parameter("affine_parameters.cy", m_center_y);
