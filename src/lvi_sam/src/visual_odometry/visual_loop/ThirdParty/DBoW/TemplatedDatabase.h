...
{ 
  ...
  // std::string temp = name + std::string(".nEntries");
  // node->declare_parameter(temp, 1);
  // node->get_parameter(temp, m_nentries);

  // temp = name + std::string(".usingDI");
  // node->declare_parameter(temp, 1);
  // node->get_parameter(temp, m_use_di);
  // m_use_di = (int)m_use_di != 0;

  // temp = name + std::string(".diLevels");
  // node->declare_parameter(temp, 1);
  // node->get_parameter(temp, m_dilevels);
  
  // // m_nentries = (int)fdb["nEntries"]; 
  // // m_dilevels = (int)fdb["diLevels"];
  
  // // cv::FileNode fn = fdb["invertedIndex"];
  // std::string temp2 = name + std::string(".invertedIndex.");
  // for(WordId wid = 0; wid < fn.size(); ++wid)
  // {
  //   // cv::FileNode fw = fn[wid];
  //   std::string temp3 = temp2 + std::to_string(wid);
    
  //   for(unsigned int i = 0; i < fw.size(); ++i)
  //   {
  //     std::string temp4 = temp3 + std::string(".imageId");
  //     node->declare_parameter(temp4, 1);
  //     node->get_parameter(temp4, m_dilevels);
