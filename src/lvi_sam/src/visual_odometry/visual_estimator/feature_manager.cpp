...
{
    RCLCPP_DEBUG(this->get_logger(), "debug show");
    for (auto &it : feature)
    {
        ...
        RCLCPP_DEBUG(this->get_logger(), "%d,%d,%d ", it.feature_id, it.used_num, it.start_frame);
        int sum = 0;
        for (auto &j : it.feature_per_frame)
        {
            RCLCPP_DEBUG(this->get_logger(), "%d,", int(j.is_used));
            sum += j.is_used;
...
{
    RCLCPP_ERROR(this->get_logger(), "Error in removeOutlier()");
    rclcpp::shutdown();
    int i = -1;
    for (auto it = feature.begin(), it_next = feature.begin();
         it != feature.end(); it = it_next)
